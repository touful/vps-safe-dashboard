...
- **Web 面板**：深海墨色与薄荷青的 SENTRY 控制台，桌面侧栏 / 窄屏横向导航，四页（总览 / 连接 / 攻击 / 导出）；五项 KPI、风险进度仪表、攻击三通道趋势（入站探测 / 拦截 / SSH 失败）、TOP 来源与端口、全球攻击地图（GeoIP）、蜜罐凭据与字典（密码默认遮蔽）、明细排序与联动过滤、时间范围切换（1h / 24h / 7d / 30d）、WS 实时推送与轮询兜底。视觉规则见 [前端视觉规范](docs/前端视觉规范.md)。
